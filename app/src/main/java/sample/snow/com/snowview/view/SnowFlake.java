...
class SnowFlake {
  ...
  private static final float FLAKE_SIZE_UPPER = 17f;
  ...
  public static SnowFlake create(Bitmap snowBitmap, int width, int height, Paint paint) {
    ...
    float x = (float) random.getRandom(flakeSize, width - flakeSize);
    float y = (float) random.getRandom(-flakeSize, height + flakeSize);
  ...
  SnowFlake(Bitmap snowBitmap, Random random, PointF position, float angle, float increment,
      float flakeSize, Paint paint) {
    this.random = random;
  ...
  private void move(int width, int height) {
    ...
    rotateAngle %= 360;
    position.set(x, y);
    desRect.set((float) (x - flakeSize), (float) (y - flakeSize), (float) (x + flakeSize),
        (float) (y + flakeSize));
  ...
  private boolean isInside(int width, int height) {
    ...
    //左 右 下
    return (x + flakeSize > 1) && (x - flakeSize < width) && (y - flakeSize < height) && (y
        >= -flakeSize - 1);
  }
